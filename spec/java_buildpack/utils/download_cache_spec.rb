...
describe JavaBuildpack::DownloadCache do
  ...
  it 'should download from a uri if the cached file does not exist' do
    ...
    Dir.mktmpdir do |root|
      JavaBuildpack::DownloadCache.new(root).get('foo', 'http://foo-uri/') {}
  ...
  it 'should pass read-only file to block' do
    ...
    Dir.mktmpdir do |root|
      JavaBuildpack::DownloadCache.new(root).get('foo', 'http://foo-uri/') do |file|
  ...
  def expect_file_deleted(extension)
    Dir.mktmpdir do |root|
      ...
      JavaBuildpack::DownloadCache.new(root).evict('foo')
