...
describe JavaBuildpack::Framework::AppDynamicsAgent do
  ...
  context do
    ...
    context do
      ...
      context do
        let(:credentials) { super().merge 'tier-name' => 'tier-from-service' }

        it 'should override the default tier-name from configuration' do
          component.release

          expect(java_opts).to include('-Dappdynamics.agent.tierName=tier-from-service')
        end
      end
