...
describe JavaBuildpack::Diagnostics::LoggerFactory do
  ...
  it 'should act as a singleton' do
    expect(described_class.get_logger).to equal(logger)
    expect(described_class.get_logger).to equal(described_class.get_logger)
  end
  ...
  it 'should not send debug logs to standard output when debug is disabled',
    ...
    logger.debug { log_message }
    expect(stderr.string).not_to match /#{log_message}/
  end
  ...
  context do
    ...
    it 'should take the default log level from a YAML file' do
      described_class.create_logger(application).debug(log_message)
      expect(stderr.string).to match /#{log_message}/
    end
  end
  ...
  it 'should fail if a LoggerFactory is constructed' do
    expect { described_class.new }.to raise_error /private method `new'/
  end
  ...
  it 'should send info logs to buildpack.log when info is enabled',
    ...
    logger.info(log_message)
    expect((app_dir + '.buildpack-diagnostics/buildpack.log').read).to match /#{log_message}/
  end

  it 'should issue warnings if the logger is re-created',
     log_level: 'WARN' do

    described_class.create_logger application
  ...
  context do
    ...
    it 'should fail if a non-existent logger is requested' do
      expect { described_class.get_logger }.to raise_error /no logger/
