...
describe JavaBuildpack::Jre::WeightBalancingMemoryHeuristic do
  ...
  TEST_WEIGHTINGS = heuristics({'heap' => TEST_HEAP_WEIGHTING, 'permgen' => TEST_PERMGEN_WEIGHTING, 'stack' => TEST_STACK_WEIGHTING, 'native' => TEST_NATIVE_WEIGHTING})
  CONFIG_FILENAME = 'jres.yml'
  ...
  it 'should fail if the configured weightings sum to more than 1' do
    with_memory_limit('1m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(heuristics({'heap' => 0.5, 'permgen' => 0.4, 'stack' => 0.1, 'native' => 0.1}))
      expect { JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {}) }.to raise_error(/Invalid/)
    end
  end
  ...
  it 'should fail if the heap weighting is less than 0' do
    with_memory_limit('1m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(heuristics({'heap' => -0.1, 'permgen' => 0.3,
  ...
  it 'should fail if the permgen weighting is less than 0' do
    with_memory_limit('1m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(heuristics({'heap' => 0.5, 'permgen' => -0.3, 'stack' => 0.1, 'native' => 0.1}))
      expect { JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {}) }.to raise_error(/Invalid/)
    end
  end
  ...
  it 'should fail if the stack weighting is less than 0' do
    with_memory_limit('1m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(heuristics({'heap' => 0.5, 'permgen' => 0.3,
                                                               'stack' => -0.1, 'native' => 0.1}))
      expect { JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {}) }.to raise_error(/Invalid/)
    end
  end
  ...
  it 'should fail if a configured weighting is invalid' do
    with_memory_limit('1m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(heuristics({'heap' => TEST_HEAP_WEIGHTING, 'permgen' => TEST_PERMGEN_WEIGHTING, 'stack' => TEST_STACK_WEIGHTING, 'native' => 'x'}))
      expect { JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {}) }.to raise_error(/Invalid/)
    end
  end
  ...
  it 'should default maximum heap size and permgen size according to the configured weightings when the weightings sum to less than 1' do
    with_memory_limit('1024m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(heuristics({'heap' => TEST_HEAP_WEIGHTING, 'permgen' => TEST_PERMGEN_WEIGHTING, 'stack' => TEST_STACK_WEIGHTING, 'native' => TEST_SMALL_NATIVE_WEIGHTING}))
      memory_heuristics = JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {})
      expect(memory_heuristics.output['heap']).to eq("#{(1024 * TEST_HEAP_WEIGHTING).to_i.to_s}M")
      expect(memory_heuristics.output['permgen']).to eq("#{(1024 * 1024 * TEST_PERMGEN_WEIGHTING).to_i.to_s}K")
    end
  ...
  it 'should default permgen size according to the configured weightings when maximum heap size is specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {'heap' => "#{(4096 * 3 / 4).to_i.to_s}m"})
  ...
  it 'should default maximum heap size according to the configured weightings when maximum permgen size is specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {'permgen' => "#{(4096 / 2).to_i.to_s}m"})
  ...
  it 'should default maximum heap size and permgen size according to the configured weightings when thread stack size is specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {'stack' => '2m'})
  ...
  it 'should default permgen size according to the configured weightings when maximum heap size and thread stack size are specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {'heap' => "#{(4096 * 3 / 4).to_i.to_s}m", 'stack' => '2m'})
  ...
  it 'should not apply any defaults when maximum heap size, maximum permgen size, and thread stack size are specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {'heap' => '1m', 'permgen' => '1m', 'stack' => '2m'})
  ...
  it 'should work correctly with a single memory type' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(heuristics({'heap' => TEST_HEAP_WEIGHTING}))
      memory_heuristics = JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {})
  ...
  it 'should work correctly with no memory types' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(CONFIG_FILE_PATH).and_return(heuristics({}))
      memory_heuristics = JavaBuildpack::Jre::WeightBalancingMemoryHeuristic.new(CONFIG_FILENAME, WEIGHTINGS_NAME, {})
