...
describe JavaBuildpack::Jre::MemoryBucket do
  ...
  TEST_WEIGHTING = 0.5
  TEST_SIZE = JavaBuildpack::Jre::MemorySize.new('10M')
  TEST_TOTAL_MEMORY = JavaBuildpack::Jre::MemorySize.new('1G')
  TEST_TOTAL_EXCESS = JavaBuildpack::Jre::MemorySize.new('200B')
  ...
  it 'should return a zero excess if the size has not been set' do
    memory_bucket = JavaBuildpack::Jre::MemoryBucket.new(TEST_NAME, TEST_WEIGHTING, nil, true, TEST_TOTAL_MEMORY)
    expect(memory_bucket.excess).to eq(JavaBuildpack::Jre::MemorySize.ZERO)
  end
  ...
  it 'should return an adjustable weighting of zero if it is not adjustable' do
    memory_bucket = JavaBuildpack::Jre::MemoryBucket.new(TEST_NAME, TEST_WEIGHTING, TEST_SIZE, false, TEST_TOTAL_MEMORY)
  ...
  it 'should adjust the size correctly if it is adjustable and the total adjustable weighting is 0' do
    ...
    expect(memory_bucket.size).to eq(JavaBuildpack::Jre::MemorySize.ZERO)
  end
end