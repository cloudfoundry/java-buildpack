...
describe JavaBuildpack::MemoryHeuristicsOpenJDK do
  ...
  it 'should fail if the configured weightings sum to more than 1' do
    with_memory_limit('1m') do
      ...
      expect { JavaBuildpack::MemoryHeuristicsOpenJDK.new({}) }.to raise_error(/Invalid/)
    end
  end
  ...
  it 'should default maximum heap size and metaspace size according to the configured weightings' do
    with_memory_limit('1024m') do
      ...
      memory_heuristics = JavaBuildpack::MemoryHeuristicsOpenJDK.new({})
  ...
  it 'should default metaspace size according to the configured weightings when maximum heap size is specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(File.expand_path OPENJDK_CONFIG_FILE_PATH).and_return(OPENJDK_TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::MemoryHeuristicsOpenJDK.new({'heap' => "#{(4096 * 3 / 4).to_i.to_s}m"})
      expect(memory_heuristics.heap).to eq("3G")
      expect(memory_heuristics.metaspace).to eq("#{(1024 * 4096 * OPENJDK_TEST_METASPACE_WEIGHTING - 1024 * 1024 * OPENJDK_TEST_METASPACE_WEIGHTING / (OPENJDK_TEST_METASPACE_WEIGHTING + OPENJDK_TEST_NATIVE_WEIGHTING)).to_i.to_s}K")
    end
  end
  ...
  it 'should default maximum heap size according to the configured weightings when maximum metaspace size is specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(File.expand_path OPENJDK_CONFIG_FILE_PATH).and_return(OPENJDK_TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::MemoryHeuristicsOpenJDK.new({'metaspace' => "#{(4096 / 2).to_i.to_s}m"})
      expect(memory_heuristics.metaspace).to eq("2G")
      expect(memory_heuristics.heap).to eq("#{(1024 * 4096 * OPENJDK_TEST_HEAP_WEIGHTING - 1024 * 4096 * 0.2 * OPENJDK_TEST_HEAP_WEIGHTING / (OPENJDK_TEST_HEAP_WEIGHTING + OPENJDK_TEST_NATIVE_WEIGHTING)).to_i.to_s}K")
    end
  end
  ...
  it 'should default maximum heap size and metaspace size according to the configured weightings when thread stack size is specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(File.expand_path OPENJDK_CONFIG_FILE_PATH).and_return(OPENJDK_TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::MemoryHeuristicsOpenJDK.new({'stack' => '2m'})
      ...
      expect(memory_heuristics.heap).to eq("#{(1024 * 4096 * OPENJDK_TEST_HEAP_WEIGHTING - 1024 * 409.6 * OPENJDK_TEST_HEAP_WEIGHTING / (OPENJDK_TEST_HEAP_WEIGHTING + OPENJDK_TEST_METASPACE_WEIGHTING + OPENJDK_TEST_NATIVE_WEIGHTING)).to_i.to_s}K")
      expect(memory_heuristics.metaspace).to eq("#{(1024 * 4096 * OPENJDK_TEST_METASPACE_WEIGHTING - 1024 * 409.6 * OPENJDK_TEST_METASPACE_WEIGHTING / (OPENJDK_TEST_METASPACE_WEIGHTING + OPENJDK_TEST_HEAP_WEIGHTING + OPENJDK_TEST_NATIVE_WEIGHTING)).to_i.to_s}K")
    end
  end
  ...
  it 'should default metaspace size according to the configured weightings when maximum heap size and thread stack size are specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(File.expand_path OPENJDK_CONFIG_FILE_PATH).and_return(OPENJDK_TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::MemoryHeuristicsOpenJDK.new({'heap' => "#{(4096 * 3 / 4).to_i.to_s}m", 'stack' => '2m'})
      ...
      expect(memory_heuristics.metaspace).to eq("#{(1024 * 4096 * OPENJDK_TEST_METASPACE_WEIGHTING - 1024 * 1024 * OPENJDK_TEST_METASPACE_WEIGHTING / (OPENJDK_TEST_METASPACE_WEIGHTING + OPENJDK_TEST_NATIVE_WEIGHTING) -
  ...
  it 'should not apply any defaults when maximum heap size, maximum metaspace size, and thread stack size are specified' do
    with_memory_limit('4096m') do
      YAML.stub(:load_file).with(File.expand_path OPENJDK_CONFIG_FILE_PATH).and_return(OPENJDK_TEST_WEIGHTINGS)
      memory_heuristics = JavaBuildpack::MemoryHeuristicsOpenJDK.new({'heap' => '1m', 'metaspace' => '1m', 'stack' => '2m'})
      expect(memory_heuristics.heap).to eq('1M')
      expect(memory_heuristics.metaspace).to eq('1M')
