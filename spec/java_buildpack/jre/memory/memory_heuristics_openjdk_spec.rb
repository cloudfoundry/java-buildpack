...
module JavaBuildpack::Jre
  ...
  describe MemoryHeuristicsOpenJDK do
    ...
    let(:openjdk_test_weightings) { heuristics({
      'heap' => OPENJDK_TEST_HEAP_WEIGHTING,
      'metaspace' => OPENJDK_TEST_METASPACE_WEIGHTING,
      'stack' => OPENJDK_TEST_STACK_WEIGHTING,
      'native' => OPENJDK_TEST_NATIVE_WEIGHTING
      }) }
    ...
    it 'should fail if the configured weightings sum to more than 1' do
      with_memory_limit('1m') do
        expect { MemoryHeuristicsOpenJDK.new({}, heuristics({'heap' => 0.5, 'metaspace' => 0.4, 'stack' => 0.1, 'native' => 0.1})) }
    ...
    it 'should fail if the heap weighting is less than 0' do
      with_memory_limit('1m') do
        expect { MemoryHeuristicsOpenJDK.new({}, heuristics({'heap' => -0.1, 'metaspace' => 0.3, 'stack' => 0.1, 'native' => 0.1})) }
          .to raise_error(/Invalid/)
      end
    end
    ...
    it 'should fail if the metaspace weighting is less than 0' do
      with_memory_limit('1m') do
        expect { MemoryHeuristicsOpenJDK.new({}, heuristics({'heap' => 0.5, 'metaspace' => -0.3, 'stack' => 0.1, 'native' => 0.1})) }
          .to raise_error(/Invalid/)
      end
    end
    ...
    it 'should fail if a configured weighting is invalid' do
      with_memory_limit('1m') do
        expect { MemoryHeuristicsOpenJDK.new({}, heuristics({'heap' => OPENJDK_TEST_HEAP_WEIGHTING, 'metaspace' => OPENJDK_TEST_METASPACE_WEIGHTING, 'stack' => OPENJDK_TEST_STACK_WEIGHTING, 'native' => 'x'})) }
          .to raise_error(/Invalid/)
      end
    end
    ...
    it 'should default maximum heap size and metaspace size according to the configured weightings' do
      with_memory_limit('1024m') do
        memory_heuristics = MemoryHeuristicsOpenJDK.new({}, openjdk_test_weightings)
    ...
    it 'should default maximum heap size and metaspace size according to the configured weightings when the weightings sum to less than 1' do
      with_memory_limit('1024m') do
        memory_heuristics = MemoryHeuristicsOpenJDK.new({}, heuristics({'heap' => OPENJDK_TEST_HEAP_WEIGHTING, 'metaspace' => OPENJDK_TEST_METASPACE_WEIGHTING, 'stack' => OPENJDK_TEST_STACK_WEIGHTING, 'native' => OPENJDK_TEST_SMALL_NATIVE_WEIGHTING}))
        expect(memory_heuristics.output['heap']).to eq("#{(1024 * OPENJDK_TEST_HEAP_WEIGHTING).to_i.to_s}M")
        expect(memory_heuristics.output['metaspace']).to eq("#{(1024 * 1024 * OPENJDK_TEST_METASPACE_WEIGHTING).to_i.to_s}K")
      end
    end
    ...
    it 'should default metaspace size according to the configured weightings when maximum heap size is specified' do
      with_memory_limit('4096m') do
        memory_heuristics = MemoryHeuristicsOpenJDK.new({'heap' => "#{(4096 * 3 / 4).to_i.to_s}m"}, openjdk_test_weightings)
    ...
    it 'should default maximum heap size according to the configured weightings when maximum metaspace size is specified' do
      with_memory_limit('4096m') do
        memory_heuristics = MemoryHeuristicsOpenJDK.new({'metaspace' => "#{(4096 / 2).to_i.to_s}m"}, openjdk_test_weightings)
    ...
    it 'should default maximum heap size and metaspace size according to the configured weightings when thread stack size is specified' do
      with_memory_limit('4096m') do
        memory_heuristics = MemoryHeuristicsOpenJDK.new({'stack' => '2m'}, openjdk_test_weightings)
    ...
    it 'should default metaspace size according to the configured weightings when maximum heap size and thread stack size are specified' do
      with_memory_limit('4096m') do
        memory_heuristics = MemoryHeuristicsOpenJDK.new({'heap' => "#{(4096 * 3 / 4).to_i.to_s}m", 'stack' => '2m'}, openjdk_test_weightings)
    ...
    it 'should not apply any defaults when maximum heap size, maximum metaspace size, and thread stack size are specified' do
      with_memory_limit('4096m') do
        memory_heuristics = MemoryHeuristicsOpenJDK.new({'heap' => '1m', 'metaspace' => '1m', 'stack' => '2m'}, openjdk_test_weightings)
