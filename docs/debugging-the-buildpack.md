...
Detect - `` alias detect='$BUILDPACK_ROOT/bin/detect .' ``

Compile - `` alias compile='$BUILDPACK_ROOT/bin/compile . $TMPDIR' ``

Release - `` alias release='$BUILDPACK_ROOT/bin/release . | ruby -e "require \"yaml\"; puts YAML.load(STDIN.read)[\"default_process_types\"][\"web\"]"' ``
